...
function find_for_template_in_string(string, template)
{
	...
	var hash_sum_of_template = count_the_sum_of_codes_of_string(template, length_of_template);
	var hash_sum_of_string = count_the_sum_of_codes_of_string(string, length_of_template);
	...
	while (finish < length_of_string)
	{
		...
		start++; finish++;
		hash_sum_of_string += string.charCodeAt(finish) - string.charCodeAt(start - 1);
	}
...
function count_the_sum_of_codes_of_string(string, max_side)
{
	var sum_of_codes = 0;	
	for (var index = 0; index < max_side; index++)
		sum_of_codes += string.charCodeAt(index);
	return sum_of_codes;
}
