...
function find_for_template_in_string(string, template)
{
	...
	var paradoxes = 0, similarity = 0, index = 0;
	...
	while (index - 1 + length_of_template < length_of_string)
	{
		if (hash_sum_of_template == hash_sum_of_string) 
		{
			if (string.substring(index, index + length_of_template) != template) paradoxes++; 
			else 
			{
		...
		hash_sum_of_string = (hash_sum_of_string - string.charCodeAt(index) * Math.pow(2, length_of_template - 1)) * 2 + string.charCodeAt(index + length_of_template);
		index++;
	}
...
function count_the_sum_of_square_codes_of_string(string, max_side)
{
	...
	for (var index = 0; index < max_side; index++)
		sum_of_codes += string.charCodeAt(index) * Math.pow(2, max_side - 1 - index);
	return sum_of_codes;
}
